 ...
 * SOFTWARE.
 */
package com.oracle.truffle.wasm.tck;
...
public class WasmTCKLanguageProvider implements LanguageProvider {
    private static final String ID = "wasm";
    ...
    public Collection<? extends Snippet> createScripts(Context context) {
        final Collection<Snippet> res = new ArrayList<>();
        res.add(loadScript(context, "resources/Ackermann.wasm", TypeDescriptor.NULL, null));
        res.add(loadScript(context, "resources/Fibonacci.wasm", TypeDescriptor.NULL, null));
        return Collections.unmodifiableCollection(res);
    }
    ...
    public Collection<? extends Source> createInvalidSyntaxScripts(Context context) {
        try {
            ...
            res.add(createSource("resources/InvalidSyntax01.wasm"));
            res.add(createSource("resources/InvalidSyntax02.wasm"));
            return Collections.unmodifiableCollection(res);
    ...
    private static Source createSource(final String resourceName) throws IOException {
        ...
        final Reader in = new InputStreamReader(WasmTCKLanguageProvider.class.getResourceAsStream(resourceName), "UTF-8");
