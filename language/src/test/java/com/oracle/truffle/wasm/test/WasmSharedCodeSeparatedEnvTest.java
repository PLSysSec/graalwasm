...
public class WasmSharedCodeSeparatedEnvTest {
    ...
    public void initializeEngines() {
        ...
        int instances = WasmLanguage.counter;
        // @formatter:off
        e1 = Context.newBuilder("wasm").engine(engine).out(os1).allowPolyglotAccess(PolyglotAccess.ALL).build();
        e1.getPolyglotBindings().putMember("extra", 1);
        e2 = Context.newBuilder("wasm").engine(engine).out(os2).allowPolyglotAccess(PolyglotAccess.ALL).build();
        e2.getPolyglotBindings().putMember("extra", 2);
        e1.initialize("wasm");
        e2.initialize("wasm");
        assertEquals("One WasmLanguage instance created", instances + 1, WasmLanguage.counter);
    }
    ...
    public void shareCodeUseDifferentOutputStreams() throws Exception {
        ...
        e1.eval("wasm", sayHello);
        assertEquals("Ahoj1\n", os1.toString("UTF-8"));
        assertEquals("", os2.toString("UTF-8"));

        e2.eval("wasm", sayHello);
        assertEquals("Ahoj1\n", os1.toString("UTF-8"));
        assertEquals("Ahoj2\n", os2.toString("UTF-8"));
    }
