...
public class WasmJavaInteropConversionTest {
    ...
    public void testGR7318Object() throws Exception {
        ...
        try (Context context = Context.newBuilder(WasmLanguage.ID).build()) {
            context.eval(Source.newBuilder(WasmLanguage.ID, sourceText, "Test").build());
            Value test = context.getBindings(WasmLanguage.ID).getMember("test");
    ...
    public void testGR7318List() throws Exception {
        String sourceText = "function test(validator, array) {\n" +
                        ...
                        "}";
        try (Context context = Context.newBuilder(WasmLanguage.ID).allowHostAccess(HostAccess.ALL).build()) {
            context.eval(Source.newBuilder(WasmLanguage.ID, sourceText, "Test").build());
            Value test = context.getBindings(WasmLanguage.ID).getMember("test");
