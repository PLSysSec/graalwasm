...
import com.oracle.truffle.wasm.test.WasmTestRunner.TestCase;

public class WasmTestRunner extends ParentRunner<TestCase> {

    private static final String SOURCE_SUFFIX = ".wat";
    ...
    public WasmTestRunner(Class<?> runningClass) throws InitializationError {
        super(runningClass);
        try {
    ...
    protected static List<TestCase> createTests(final Class<?> c) throws IOException, InitializationError {
        WasmTestSuite suite = c.getAnnotation(WasmTestSuite.class);
        if (suite == null) {
            throw new InitializationError(String.format("@%s annotation required on class '%s' to run with '%s'.", WasmTestSuite.class.getSimpleName(), c.getName(), WasmTestRunner.class.getSimpleName()));
        }

        String[] paths = suite.value();

        Class<?> testCaseDirectory = c;
        if (suite.testCaseDirectory() != WasmTestSuite.class) {
    ...
    private static final List<NodeFactory<? extends WasmBuiltinNode>> builtins = new ArrayList<>();
    ...
    protected void runChild(TestCase testCase, RunNotifier notifier) {
        ...
        try {
            ...
            for (NodeFactory<? extends WasmBuiltinNode> builtin : builtins) {
                WasmLanguage.installBuiltin(builtin);
            }
    ...
    private static void run(Context context, Path path, PrintWriter out) throws IOException {
        try {
            /* Parse the Wasm source file. */
            Source source = Source.newBuilder(WasmLanguage.ID, path.toFile()).interactive(true).build();
    ...
    public static void runInMain(Class<?> testClass, String[] args) throws InitializationError, NoTestsRemainException {
        ...
        WasmTestRunner suite = new WasmTestRunner(testClass);
        if (args.length > 0) {
