...
public class WasmExceptionTest {

    private Context ctx;

    @Before
    public void setUp() {
        this.ctx = Context.create("wasm");
    }
    ...
    private void assertException(boolean failImmediately, String source, String... expectedFrames) {
        ...
        try {
            Value value = ctx.eval("wasm", source);
    ...
    private static void assertFrames(boolean isEval, PolyglotException e, String... expectedFrames) {
        ...
        for (StackFrame frame : e.getPolyglotStackTrace()) {
            if (i < expectedFrames.length && expectedFrames[i] != null) {
                ...
                Assert.assertEquals("wasm", frame.getLanguage().getId());
    ...
    public void testGuestLanguageError() {
        try {
            String source = "function bar() { x = 1 / \"asdf\"; }\n" +
                            ...
                            "function main() { foo(); }";
            ctx.eval(Source.newBuilder("wasm", source, "script.wasm").buildLiteral());
            fail();
        } catch (PolyglotException e) {
            assertTrue(e.isGuestException());

            Iterator<StackFrame> frames = e.getPolyglotStackTrace().iterator();
            assertGuestFrame(frames, "wasm", "bar", "script.wasm", 21, 31);
            assertGuestFrame(frames, "wasm", "foo", "script.wasm", 59, 64);
            assertGuestFrame(frames, "wasm", "main", "script.wasm", 86, 91);
            assertHostFrame(frames, Context.class.getName(), "eval");
            assertHostFrame(frames, WasmExceptionTest.class.getName(), "testGuestLanguageError");
    ...
    public void testProxyGuestLanguageStack() {
        Value bar = ctx.eval("wasm", "function foo(f) { f(); } function bar(f) { return foo(f); } function main() { return bar; }");
    ...
    private static void assertProxyException(TestProxy proxy, PolyglotException e) {
        ...
        for (int i = 0; i < 2; i++) {
            assertGuestFrame(frames, "wasm", "foo", "Unnamed", 18, 21);
            assertGuestFrame(frames, "wasm", "bar", "Unnamed", 50, 56);
        ...
        assertHostFrame(frames, Value.class.getName(), "execute");
        assertHostFrame(frames, WasmExceptionTest.class.getName(), "testProxyGuestLanguageStack");
