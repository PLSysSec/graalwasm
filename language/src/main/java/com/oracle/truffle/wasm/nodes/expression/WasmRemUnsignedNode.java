 ...
 * the code simple.
 */
@NodeInfo(shortName = "rem_u")
public abstract class WasmRemUnsignedNode extends WasmBinaryNode {

    @Specialization
    protected int rem_u(int left, int right) {
        if (right == 0) {
            return 0; // FIXME undefined
        }
        int result = Integer.remainderUnsigned(left, right); // TODO correctly truncated?
        return result;
    }

    @Specialization
    protected long rem_u(long left, long right) {
        if (right == 0) {
            return 0; // FIXME undefined
        }
        long result = Long.remainderUnsigned(left, right); // TODO correctly truncated?
        return result;
    }
