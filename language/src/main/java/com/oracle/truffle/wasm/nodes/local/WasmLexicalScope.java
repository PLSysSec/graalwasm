...
/**
 * Wasm language lexical scope. There can be a block scope, or function scope.
 */
public final class WasmLexicalScope {

    private final Node current;
    private final WasmBlockNode block;
    private final WasmBlockNode parentBlock;
    private final RootNode root;
    private WasmLexicalScope parent;
    private Map<String, FrameSlot> varSlots;

    /**
     * Create a new block Wasm lexical scope.
     *
     * @param current the current node
     * @param block a nearest block enclosing the current node
     * @param parentBlock a next parent block
     */
    private WasmLexicalScope(Node current, WasmBlockNode block, WasmBlockNode parentBlock) {
    ...
    /**
     * Create a new functional Wasm lexical scope.
     *
     ...
     */
    private WasmLexicalScope(Node current, WasmBlockNode block, RootNode root) {
        this.current = current;
        this.block = block;
    ...
    public static WasmLexicalScope createScope(Node node) {
        WasmBlockNode block = getParentBlock(node);
        if (block == null) {
            // We're in the root.
            block = findChildrenBlock(node);
            if (block == null) {
                // Corrupted Wasm AST, no block was found
                assert node.getRootNode() instanceof WasmEvalRootNode : "Corrupted Wasm AST under " + node;
                return new WasmLexicalScope(null, null, (WasmBlockNode) null);
            }
            node = null; // node is above the block
        }
        // Test if there is a parent block. If not, we're in the root scope.
        WasmBlockNode parentBlock = getParentBlock(block);
        if (parentBlock == null) {
            return new WasmLexicalScope(node, block, block.getRootNode());
        } else {
            return new WasmLexicalScope(node, block, parentBlock);
        }
    }

    private static WasmBlockNode getParentBlock(Node node) {
        WasmBlockNode block;
        Node parent = node.getParent();
        // Find a nearest block node.
        while (parent != null && !(parent instanceof WasmBlockNode)) {
            parent = parent.getParent();
        }
        if (parent != null) {
            block = (WasmBlockNode) parent;
        } else {
            block = null;
        }
        return block;
    ...
    private static WasmBlockNode findChildrenBlock(Node node) {
        WasmBlockNode[] blockPtr = new WasmBlockNode[1];
        node.accept(new NodeVisitor() {
            @Override
            public boolean visit(Node n) {
                if (n instanceof WasmBlockNode) {
                    blockPtr[0] = (WasmBlockNode) n;
                    return false;
                } else {
                    return true;
                }
    ...
    public WasmLexicalScope findParent() {
        if (parentBlock == null) {
        ...
        if (parent == null) {
            Node node = block;
            WasmBlockNode newBlock = parentBlock;
            // Test if there is a next parent block. If not, we're in the root scope.
            WasmBlockNode newParentBlock = getParentBlock(newBlock);
            if (newParentBlock == null) {
                parent = new WasmLexicalScope(node, newBlock, newBlock.getRootNode());
            } else {
                parent = new WasmLexicalScope(node, newBlock, newParentBlock);
            }
        }
        return parent;
    }
    ...
    private Map<String, FrameSlot> collectVars(Node varsBlock, Node currentNode) {
        ...
        // with WasmWriteLocalVariableNode. The traversal stops when we hit the current node.
        ...
        NodeUtil.forEachChild(varsBlock, new NodeVisitor() {
            ...
            public boolean visit(Node node) {
                ...
                if (!(node instanceof WasmBlockNode)) {
                ...
                if (node instanceof WasmWriteLocalVariableNode) {
                    WasmWriteLocalVariableNode wn = (WasmWriteLocalVariableNode) node;
    ...
    private static Map<String, FrameSlot> collectArgs(Node block) {
        ...
        // To collect argument slots, search for WasmReadArgumentNode inside of
        // WasmWriteLocalVariableNode.
        Map<String, FrameSlot> args = new LinkedHashMap<>(4);
        NodeUtil.forEachChild(block, new NodeVisitor() {

            private WasmWriteLocalVariableNode wn; // The current write node containing a slot

            @Override
            public boolean visit(Node node) {
                // When there is a write node, search for WasmReadArgumentNode among its children:
                if (node instanceof WasmWriteLocalVariableNode) {
                    wn = (WasmWriteLocalVariableNode) node;
                    boolean all = NodeUtil.forEachChild(node, this);
                    wn = null;
                    return all;
                } else if (wn != null && (node instanceof WasmReadArgumentNode)) {
                    ...
                    args.put(name, slot);
                    return true;
                } else if (wn == null && (node instanceof WasmStatementNode)) {
                    // A different Wasm node - we're done.
                    return false;
                } else {
