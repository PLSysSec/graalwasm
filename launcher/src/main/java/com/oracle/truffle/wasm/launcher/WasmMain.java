 ...
 * SOFTWARE.
 */
package com.oracle.truffle.wasm.launcher;
...
public final class WasmMain {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (file == null) {
            // @formatter:off
            source = Source.newBuilder(Wasm, new InputStreamReader(System.in), "<stdin>").build();
            // @formatter:on
        } else {
            source = Source.newBuilder(Wasm, new File(file)).build();
        }
    ...
    private static int executeSource(Source source, InputStream in, PrintStream out, Map<String, String> options) {
        ...
        try {
            context = Context.newBuilder(Wasm).in(in).out(out).options(options).build();
